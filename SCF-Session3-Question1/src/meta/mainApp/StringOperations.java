...
import meta.helper.StringImplementation;
...
public class StringOperations {
	...
	public static void main(String[] args) {
		...
		StringImplementation stringObject = new StringImplementation();
		// object of service class 
		int messageLength = Messages.ARRAY_CHOICE_MESSAGE.length;
		/*
		 * printing User Choice Messages
		 */
		for(int i=0; i<messageLength; i++)
		{
			String message = Messages.ARRAY_CHOICE_MESSAGE[i];
		...
		switch(userChoice){
			case 'a':
			case 'A': stringOne = getInput(Messages.FIRST_STRING_MESSAGE);
					  // calling getInput(message) for taking user input and printing messages 
					  stringTwo = getInput(Messages.SECOND_STRING_MESSAGE);
					  ...
					  String message = (isEqual==1)?Messages.STRING_ISEQUAL_TRUE:Messages.STRING_ISEQUAL_FALSE;
					  System.out.println(message);
				break;
			case 'b':
			case 'B': stringOne = getInput(Messages.STRING_INPUT_MESSAGE);
					  String reverseString = stringObject.reverseString(stringOne);
					  // calling function with service object to get reverse of a string
					  System.out.println(Messages.STRING_REVERSE_MESSAGE+reverseString);
				break;
			case 'c':
			case 'C': stringOne = getInput(Messages.STRING_INPUT_MESSAGE);
					  String zigZagString = stringObject.convertStringToZigZag(stringOne);
					  // calling fn with serviceObject to get zigzap word string
					  System.out.println(Messages.STRING_ZIG_ZAG_MESSAGE+zigZagString);
					  break;
			case 'd':
			case 'D': stringOne = getInput(Messages.STRING_INPUT_MESSAGE);
					  String largestWord = stringObject.largestWord(stringOne);
					  // calling fn to find largest word in a sentence
					  System.out.println(Messages.STRING_LARGET_MESSAGE+largestWord);
					  //System.out.println("We are working on this");
					  break;
			default: System.out.println(Messages.DEFAULT_STRING_MESSAGE);
		}
	}
